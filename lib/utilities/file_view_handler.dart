...
void fileViewHandler(BuildContext context,Data data,{OnChanged? onChanged})async{
  if(onChanged!=null){
    onChanged(await _addToHistory(data));
  }
  if(data.fileType.toLowerCase() == 'pdf'){
    Navigator.push(context,MaterialPageRoute(builder: (context)=>PdfViewer(data:data,onChanged: onChanged,)));
  }else {
...
Future<bool> _addToHistory(Data data)async {
  ...
  if(!data.isHistory){
    Read.updateFiles(data,typeOfUpdate: TypeOfUpdate.HISTORY);
